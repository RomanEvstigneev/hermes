...
IMPORTANT: English only. Use the date from the script header for all memory dates and for the final report heading, even if the LOG_PATH filename or cron execution date is different.
...
Secret handling: never include raw credentials, API keys, Slack token strings (`xox...` / `xapp...`), authorization headers, or private key material in memory files or the final Slack message. Replace any such material with `[REDACTED_SECRET]`.
...
FOR EACH HUMAN PERSON seen in today's messages:
...
- Do not create or update a people profile for the Hermes bot itself unless explicitly requested.
